...
public class ProductServiceImpl implements IProductService {
    ...
    public ServerResponse saveOrUpdateProduct(Product product) {
        if (product != null) {
            if (StringUtils.isNoneBlank(product.getSubImages())) {
                String[] splitedsubimages = product.getSubImages().split(",");
                if (splitedsubimages.length > 0) {
                    product.setMainImage(splitedsubimages[0]);
                }
            }
            if (product.getId() != null) {
                int count = productMapper.updateByPrimaryKey(product);
                if (count > 0)
                    return ServerResponse.createBySuccess("更新该产品成功");
                else
                    return ServerResponse.createByErrorMessage("更新产品失败");
            } else {
                int insertcount = productMapper.insert(product);
                if (insertcount > 0)
                    return ServerResponse.createBySuccess("加入该产品成功");
                else
                    return ServerResponse.createByErrorMessage("加入产品失败");
            }
        }
    ...
    public ServerResponse<String> setSaleStatus(Integer productId, Integer status) {
        ...
        int updateCount = productMapper.updateByPrimaryKeySelective(newProduct);
        if (updateCount > 0)
    ...
    public ServerResponse<ProductDetailVo> manageProductDetail(Integer productId) {
        ...
        if (product == null)
            return ServerResponse.createByErrorMessage("没有找到产品信息");
...
//        resultList.setList(productListVoList);
        PageInfo resultList=new PageInfo(productListVoList);
    ...
    public ServerResponse<PageInfo> searchProject(String productName,Integer productId,int pageNum,int pageSize){
        PageHelper.startPage(pageNum,pageSize);
